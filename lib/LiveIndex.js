...
export default class LiveIndex extends EventEmitter {
  ...
  watch () {
    this._watchPaths.forEach(path => {
      ...
      tail
        .on("data", chunk => this._evaluateChunk(path, chunk))
  ...
  _evaluateChunk (path, chunk) {
    const indexer = this._indexer
    const tail = this._tails.get(path)
  ...
  addStaticDataFile (path, cb) {
    this.addDataFileWithoutIndexing(path)
    const tail = new TailFollow(resolve(path), { follow: false, objectMode: true })
    this._tails.set(resolve(path), tail)
    tail
  ...
  readStreamFromIndex (identifier) {
    const index = this.fileAndPositionForIdentifier(identifier)
    if (index === undefined) {
      return undefined
    }

    return new CatTail(this._filesStartingAt(index.file), {
      start: index.position
    })
  }

  get files () {
    return [...this._files.entries()].map(f => f[1].path)
  }

  _filesStartingAt (file) {
    let found = false
    return this.files.filter(f => {
      if (found === true || f === file) {
        return found = true
      }
      return false
    })
