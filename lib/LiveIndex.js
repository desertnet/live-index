...
export default class LiveIndex extends EventEmitter {
  ...
  insert (identifier, filePath, position) {
    ...
    return Promise.resolve(this._index.set(identifier, {file, position}))
      .then(() => { this.emit("insert", identifier) })
  }

  insertLink (identifier, destinationId) {
    assertNoCircularLinks(this._index, identifier, destinationId)
    this._index.set(identifier, destinationId)
  }

  fileAndPositionForIdentifier (identifier) {
    let result = this._index.get(identifier)
    if (isPromise(result)) {
      return this._asyncFileAndPositionForIdentifier(result)
    }

    // Resolve links
    while (isString(result)) {
      result = this._index.get(result)
    }
  ...
  _asyncFileAndPositionForIdentifier (result) {
    return new Promise (resolve => {
      result.then(result => {
        // Resolve links
        if (isString(result)) {
          result = this._index.get(result)
          return resolve(this._asyncFileAndPositionForIdentifier(result))
        }
  ...
  readStreamBetweenIndexes (startId, endId) {
    ...
    const readStreamFromIndexes = ([startIndex, endIndex]) => {
    ...
    if (isPromise(startIndex) || isPromise(endIndex)) {
      return Promise.all([startIndex, endIndex])
        .then(indexes => readStreamFromIndexes(indexes))
    }
    else {
      return readStreamFromIndexes([startIndex, endIndex])
    }
  }
...
function assertNoCircularLinks (index, startingId, destinationId) {
  const breadCrumbs = [startingId]
  function throwErrorIfCircular (id) {
    breadCrumbs.push(id)
    if (id === startingId) {
      const trail = breadCrumbs.join(" -> ")
      throw new Error(`Attempted to insert circular link: ${trail}`)
    }
  }

  throwErrorIfCircular(destinationId)

  let indexEntry
  while (indexEntry = index.get(destinationId)) {
    if (isString(indexEntry)) {
      throwErrorIfCircular(indexEntry)
      destinationId = indexEntry
    }
    else {
      break
    }
